...
void NetService::acceptHandler(struct evconnlistener *listener, evutil_socket_t cfd, struct sockaddr *addr, int len, void *ptr) {
    GBF_PARM* pParm = static_cast<GBF_PARM*>(ptr);

    LOG(INFO) << "accept client ip:" << inet_ntoa(((struct sockaddr_in*)addr)->sin_addr) << "  prot:" << ((struct sockaddr_in*)addr)->sin_port << " fd:" << cfd;
